...
try{
  const results=await Promise.all(AREAS.map(async a=>{const r=await fetch(`./data/${a.id}.json`);if(r.status===404)return [a.id,[]];if(!r.ok)throw Error('문제를 불러오지 못했습니다. 새로고침해 주세요.');return [a.id,await r.json()];}));
  bank=Object.fromEntries(results);store=localStore(bank);read();home();
  if(location.hash.startsWith('#resume=')){const encoded=location.hash.slice(8);history.replaceState(null,'',location.pathname+location.search);await showImport(encoded);}
