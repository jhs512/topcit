...
test('all four banks have 100 unique, valid sourced questions',async()=>{
  ...
  const registry=JSON.parse(await readFile(new URL('../data/id-registry.json',import.meta.url),'utf8'));
  for(const area of AREAS){
    ...
    for(const q of bank){
      ...
      assert.ok(registry[q.id]&&!registry[q.id].retired,`ID 등록부 확인: ${q.id}`);
      assert.equal(registry[q.id].area,q.area);assert.equal(registry[q.id].revision,q.revision);
      assert.ok(q.prompt.trim().length>0,q.id);assert.ok(!prompts.has(q.prompt),q.id);prompts.add(q.prompt);
